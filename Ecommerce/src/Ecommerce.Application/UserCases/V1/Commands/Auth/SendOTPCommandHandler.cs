...
public class SendOTPCommandHandler : ICommandHandler<Command.SendOTPCommand>
{
    ...
    public async Task<Result> Handle(Command.SendOTPCommand request, CancellationToken cancellationToken)
    {
        var user = await _userRepository.FindSingleAsync(x => x.Email.Equals(request.Email));

        if (request.Type == VerificationCodeType.REGISTER && user is not null)
            return Result.Failure(new Error("Email.AlreadyExists", "Email already exists."));

        if (request.Type == VerificationCodeType.FORGOT_PASSWORD && user is null)
        ...
        await _verificationCodeRepository.UpsertAsync(request.Email, code, request.Type, expiresAt);
